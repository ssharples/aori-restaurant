  ...
  persist(
    (set, get) => ({
      ...
      startRealTimeSync: () => {
        ...
        (get() as GroupSessionStore & { _pollInterval?: NodeJS.Timeout })._pollInterval = pollInterval;
      },

      stopRealTimeSync: () => {
        const pollInterval = (get() as GroupSessionStore & { _pollInterval?: NodeJS.Timeout })._pollInterval;
        if (pollInterval) {
